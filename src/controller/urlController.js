...
const redisClient = redis.createClient(
    19837,// port 
    "redis-19837.c212.ap-south-1-1.ec2.cloud.redislabs.com", //hostname
    { no_ready_check: true }
);
redisClient.auth("ceul8MIokpMHPhOwDYe3DZENKHnI1D5z", function (err) {     //password
    if (err) throw err;
});
...
const createUrl = async function (req, res) {
    try {
        ...
        //validation of valid url through regex
        ...
        if (changeToObject) {
        return res.status(200).send({ status: true, msg: "details fetched successfully", data: urlDetails })
        }
